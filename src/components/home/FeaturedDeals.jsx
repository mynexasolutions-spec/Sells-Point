...
export default function FeaturedDeals() {
  ...
  return <section className="home-container pb-14 sm:pb-20"><div className="border-t border-ink-100 pt-14 sm:pt-16"><div className="mb-8 flex items-center justify-between gap-4"><h2 className="min-w-0 flex-1 font-display text-2xl font-bold text-ink-900 sm:text-3xl md:text-4xl">Featured Deals</h2><button type="button" onClick={() => router.push("/search")} className="inline-flex shrink-0 items-center gap-1 whitespace-nowrap text-sm font-semibold text-brand-700 hover:text-brand-800 sm:text-base">View All Deals<ArrowRight aria-hidden="true" size={18} /></button></div><div className="scrollbar-hidden flex snap-x gap-5 overflow-x-auto pb-6">{deals.map((listing) => <div key={listing.id} className="w-[84vw] max-w-[340px] shrink-0 snap-start sm:w-[320px] lg:w-[340px]"><ProductCard listing={listing} /></div>)}{deals.length === 0 && <p className="rounded-2xl border border-dashed border-ink-200 px-6 py-10 text-base text-ink-500">Featured deals will appear here as soon as listings are available.</p>}</div></div></section>;
}
